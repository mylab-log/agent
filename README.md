...
При развёртывании необходимо указать директорию с логами и настройки подключения к `Elasticsearch`. Сервис будет отслеживать пополнение логов, обрабатывать и отправлять новые записи в `Elasticsearch`. 
...
* `log` - содержит оригинальное лог-сообщение (может отсутсвовать, если все данные из оригинального лога содержатся в других полях записи)
...
Следующие поля добавляются в запись лога:

* `facts` - содержание узла `Facts` из оригинального лога в формате YAML
* `exception` - содержание узла `Exception` из оригинального лога в формате YAML
* каждый элемент списка `Labels`, кроме `log-level` и `log_level`.

Поле `log`, содержащее оригинальную лог-запись удаляется, т.к. все данные из него перенесены в поля лога.
...
Назначается автоматически, когда не удалось определить тип формата лога, т.е. если он не указан для контейнера.

При этом присходит попытка собрать многострочный лог, если лог начинается с текста, а последующие строки начинаются с пробельных символов. 

Первая строка такого лога помещается в `message`. Орагинальный лог помещается в `log`. 
